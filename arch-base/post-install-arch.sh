...
PACMAN_PACKAGES="$SCRIPT_DIR/arch-base/packages/pacman-packages.sh"
FLATPAKS="$SCRIPT_DIR/arch-base/packages/flatpaks.sh"
SNAPS="$SCRIPT_DIR/arch-base/packages/snaps.sh"
FILES="$SCRIPT_DIR/arch-base/packages/download-files.sh"
FONTS="$SCRIPT_DIR/arch-base/packages/fonts.sh"

# Check if source files exist before sourcing
for file in "$PACMAN_PACKAGES" "$FLATPAKS" "$SNAPS" "$FILES" "$FONTS"; do
    if [[ ! -f "$file" ]]; then
...
if [[ $EUID -ne 0 ]]; then
  echo "You must be a root user to run this script, please run sudo ./post-install-arch.sh" 2>&1
  exit 1
fi
...
pacman -Syu --noconfirm || echo "System upgrade failed, but proceeding with installations..."
...
# Cleanup
echo -e "\nCleaning up..."
# You can also clean the package cache to free up space
